...
import { HandleRegister, HandleGitHubRegister, HandleGoogleRegister } from '../../Register/HandleRegister';
...
export const RegisterForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (response === true) { navigate("/auth/login"); } 
    ...
    return (
        <div className={`flex flex-col sm:p-8 p-4 border border-gray-300 rounded-2xl ${loading && "pointer-events-none"}`}>
            ...
            <div>
                ...
                <p className='select-none uppercase font-light text-sm'>Sign in to continue</p>
            </div>
                    ...
                    <button type='submit' className={`w-full h-full py-2 bg-gray-800 text-white rounded-2xl hover:-translate-y-[3px] hover:shadow-2xl transition-all duration-300 cursor-pointer flex items-center gap-2 justify-center relative`}>
                        {!loading ? (
                            ...
                            <React.Fragment>
                                ...
                                <span className='text-transparent text-sm uppercase'>Sign in</span>
            ...
            <p className='mt-2 text-center text-sm text-gray-800 opacity-75'>Already have an account? <Link className="text-blue-500" to={"/auth/login"}>Login</Link></p>
            ...
            <ul className='flex gap-2 mt-2 sm:flex-row flex-col'>
                <li className='flex'><GoogleButton handle={() => HandleGoogleRegister({ navigate })} /></li>
                <li className='flex'><GithubButton handle={() => HandleGitHubRegister({ navigate })} /></li>
            </ul>
